            ...
            <div class="container">
                <div class="row">
                    <div class="col-md-8 col-md-offset-2 text-center fh5co-heading animate-box">
                        ...
                        <p>Jeżeli masz zaproszenie, nie przejmuj się. Być może Twoje dane nie zostały jeszcze
                            wprowadzone.</p>
                    </div>
                ...
                @elseif(isset($status) && $status == "after_confirmation_time")
                    ...
                    @if($data->confirmed == 0)
                        @include('after._unconfirmed')
                    @endif

                    @if($data->confirmed == 1)
                        @include('after._confirmed', ['data' => $data])
                    @endif
                            ...
                            <div class="col-md-8 col-md-offset-2 text-center fh5co-heading animate-box">
                                @if (isset($status) && $status == "data_saved")
                                    <div class="alert alert-success" role="alert">
                                        Super! Zapisaliśmy Twoje uwagi, jeżeli chcesz coś zmienić w przyszłości,
                                        wypełnij formularz na stronie głównej jeszcze raz i popraw dane.
                                    </div>
                                @endif
                                @if (isset($status) && $status == "companion_added")
                                    <div class="alert alert-success" role="alert">
                                        Twoja osoba towarzysząca została dodana, użyj imienia i nazwiska osoby
                                        towarzyszącej w formularzu na stronie głównej, aby zmienić dane.
                                    </div>
                                @endif
                                @if (isset($status) && $status == "child_added")
                                    <div class="alert alert-success" role="alert">
                                        Dziecko zostało dodane, użyj imienia i nazwiska dziecka w formularzu na stronie
                                        głównej, aby zmienić dane.
                                    </div>
                                @endif
                                ...
                                <p>Jeżeli chcesz nas poinformować o alergiach, masz jakieś uwagi <br> lub potrzebujesz
                                    transportu to wypełnij poniższy formularz!</p>
                            </div>
                            <div class="col-md-10 col-md-offset-1">
                                <a href="{{ route('main') }}">
                                    <button type="" class="btn btn-default btn-block">Powrót do strony głównej</button>
                            ...
                            @if (\App\Models\Guest::guestIsAChild($gid) == 0)
                                @if (\App\Models\Companion::companionExists($gid) == 1)
                                    ...
                                    @if (!\App\Models\Companion::companionConfirmedCheck(\App\Models\Companion::getMyCompanionId($gid)))
                                        <a href="{{ route('show.companion', ['id' => $gid]) }}">
                                            <button type="submit" class="btn btn-outline-success"><i
                                                    class="far fa-kiss-wink-heart"></i> {{ \App\Models\Companion::getNameOfCompanion($gid) }}
                                            </button>
                                        </a>
                                        <a href="{{ route('companion.confirm', ['id' => \App\Models\Companion::getMyCompanionId($gid)]) }}">
                                            <button type="submit" class="btn btn-outline-success"
                                                    style="background-color: rgba(110,211,96,0.61)"><i
                                                    class="fas fa-check"></i> Potwierdź
                                            </button>
                                        </a>
                                    ...
                                    @if(\App\Models\Companion::companionConfirmedCheck(\App\Models\Companion::getMyCompanionId($gid)))
                                        <a href="{{ route('show.companion', ['id' => $gid]) }}">
                                            <button type="submit" class="btn btn-outline-success"
                                                    style="background-color: rgba(0,187,0,0.32)"><i
                                                    class="far fa-kiss-wink-heart"></i> {{ \App\Models\Companion::getNameOfCompanion($gid) }}
                                            </button>
                                        </a>
                                        @if(\App\Models\Guest::transportDifferences($gid, \App\Models\Companion::getMyCompanionId($gid)) == 1)
                                            <p style="color: #dc3545">Twoja Osoba Towarzysząca ma zaznaczone inne opcje
                                                dotyczące transportu.</p>
                                        @endif
                                    @endif
                                @else
                                    <label for="exampleFormControlSelect1">Osoba Towarzysząca: </label><br>
                                    <a href="{{ route('add.companion', ['id' => $gid]) }}">
                                        <button type="submit" class="btn btn-outline-success"><i
                                                class="fas fa-user-plus"></i> Dodaj osobę towarzyszącą
                                        </button>
                                    </a>
                                @endif
                            @endif
                                ...
                                @if (\App\Models\Child::doIHaveAChild($gid))
                                    ...
                                    @foreach(\App\Models\Child::getChildrensData($gid) as $childs)
                                        <a href="{{ route('show.children', ['id' => $childs->id, 'gid' => $gid]) }}">
                                            <button type="submit"
                                                    style="@if($childs->confirmed == 0) @else background-color: rgba(77,192,241,0.4) @endif"
                            ...
                            @if (\App\Models\Child::amIaChild($data->id) || $data->child == 1)
                                <div class="row mt-3">
                                    <div class="col-md-12"><label class="labels">Wiek:</label>
                                        <input class="form-control" type="number" value="{{ $data->age }}"
                                               pattern="[0-30]" name="age"
